import { fetchTradingViewQuotes } from './api'
import { applyPositionQuotes, getPositions } from './store'

const loadedQuoteSymbols = new Map<string, string>()
const failedQuoteSymbols = new Set<string>()
let quotesFetchPromise: Promise<void> | null = null

function ensureQuotesLoaded(): void {
  if (quotesFetchPromise) {
    return
  }

  const positions = getPositions()
  const pending: Array<{ id: string; symbol: string }> = []

  positions.forEach(position => {
    const symbol = position.quoteSymbol?.trim()
    if (!symbol || failedQuoteSymbols.has(symbol)) {
      return
    }
    const previouslyLoaded = loadedQuoteSymbols.get(position.id)
    if (previouslyLoaded === symbol) {
      return
    }
    pending.push({ id: position.id, symbol })
  })
  ...
  const symbols = pending.map(item => item.symbol)
  const symbolToPosition = new Map(pending.map(item => [item.symbol, item.id]))

  quotesFetchPromise = fetchTradingViewQuotes({ symbols })
    .then(response => {
      const updates = response
        .map(quote => {
          const positionId = symbolToPosition.get(quote.symbol)
          if (!positionId) {
            return null
          }

          loadedQuoteSymbols.set(positionId, quote.symbol)
        ...
        .filter(
          (item): item is { id: string; symbol: string; price: number; currency?: string } => item !== null,
        )

      if (updates.length) {
        applyPositionQuotes(updates)
      }

      symbols.forEach(symbol => {
        if (!response.find(item => item.symbol === symbol)) {
          failedQuoteSymbols.add(symbol)
        }
      })
    })
    .catch(error => {
      console.error('Nie udało się pobrać notowań z TradingView:', error)
      symbols.forEach(symbol => failedQuoteSymbols.add(symbol))
    })
    .finally(() => {
      quotesFetchPromise = null
    })
}
